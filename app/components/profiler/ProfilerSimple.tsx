...
interface ProfilerSimpleProps {
    tab: string; // "profiler-simple/without" | "profiler-simple/with"
}
...
const initialStats: RenderStats = { commitCount: 0, totalActualTime: 0, lastActualTime: 0 };

const StatsPanel: React.FC<{ statsRef: React.MutableRefObject<RenderStats> }> = ({ statsRef }) => {
    const [view, setView] = useState<RenderStats>(statsRef.current);
    const lastSeen = useRef<RenderStats>(statsRef.current);
    useEffect(() => {
    ...
    return (
        <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
            <div className="rounded bg-gray-100 p-3">
                ...
                <div className="text-xl font-semibold text-gray-800">{view.commitCount}</div>
            </div>
            <div className="rounded bg-gray-100 p-3">
                <div className="text-gray-600">Total render time (ms)</div>
                <div className="text-xl font-semibold text-gray-800">{view.totalActualTime.toFixed(1)}</div>
            </div>
            <div className="rounded bg-gray-100 p-3">
                <div className="text-gray-600">Last render (ms)</div>
                <div className="text-xl font-semibold text-gray-800">{view.lastActualTime.toFixed(1)}</div>
            </div>
            <div className="col-span-3">
                <button
                    className="rounded bg-gray-200 px-3 py-2 text-sm text-gray-800 hover:bg-gray-300"
    ...
    return (
        <div className="mt-4">
            <div className="mb-2 flex items-center justify-between">
                ...
                <button onClick={copy} className="rounded bg-gray-200 px-2 py-1 text-xs text-gray-700 hover:bg-gray-300">Copy</button>
            </div>
...
function expensiveCalc(x: number) {
    let acc = 0;
...
const DisplayRaw: React.FC<{ n: number, mode: string }> = ({ n, mode }) => {
    ...
    const value = mode === "with" ? memoizedValue : (directValue as number);

    return (
        <div className="rounded border border-gray-200 p-4">
            <div className="mb-2 text-black">Computed value: <span className="font-semibold">{value}</span></div>
            <div className="mb-1 text-sm text-gray-700">Child pings: {pings}</div>
            <div className="mb-3 text-xs text-gray-600">Counts — memo: {memoCount.current}, direct: {directCount.current}</div>
            <button className="rounded bg-gray-200 px-3 py-1 text-sm text-gray-800 hover:bg-gray-300" onClick={() => setPings((c) => c + 1)}>
                Update child (Ping)
            </button>
            <button
                className="ml-2 rounded bg-gray-200 px-3 py-1 text-sm text-gray-800 hover:bg-gray-300"
...
export default function ProfilerSimple({ tab }: ProfilerSimpleProps) {
    const [, sub = "without"] = tab.split("/");
    const mode: Mode = sub === "with" ? "with" : "without";

    const [n, setN] = useState(25);
    const [unrelated, setUnrelated] = useState(0);
    const statsRef = useRef<RenderStats>(initialStats);
    

    const onRender = useCallback<React.ProfilerOnRenderCallback>((id, phase, actualDuration) => {
    ...
    return (
        <div>
            ...
            <div className="mb-4 flex flex-wrap items-center gap-3">
                <label className="text-sm text-gray-700">Input (n)</label>
                <input
                    type="range"
                    min={1}
                    max={100}
            ...
            <div className="mb-2 text-sm text-gray-600">
                Mode: <span className="font-medium text-gray-800">{mode === "with" ? "With memo + useMemo + useCallback" : "Without optimizations"}</span>
            </div>
